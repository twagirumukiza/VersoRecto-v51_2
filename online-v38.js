...
const els = {
  ...
  soundToggleOnline:document.getElementById("soundToggleOnline")
};
...
function isRanked(uid) {
  return Boolean((gameState.ranking || []).some(entry => entry.playerId === uid));
}
...
function requiredWinnersToFinish() {
  return Math.max(1, playerArray().length - 1);
}
...
function emptyGame(){return {status:"idle",maxPlayers:4,players:{},playerOrder:[],pieces:createPieces([]),turnOrder:[],currentTurnIndex:0,ranking:[],history:[],winner:null,activeUid:null,lastMoveBy:null};}
...
function moveSelected(to){
  ...
  if (checkVictory(player) && !isRanked(player.uid)) {
...
function toggleSound(){audioState.enabled=!audioState.enabled;els.soundToggleOnline.textContent=audioState.enabled?"🔊 Son":"🔇 Son";els.soundToggleOnline.setAttribute("aria-pressed",String(audioState.enabled));if(audioState.enabled)startAmbientMusic();else stopAmbientMusic();}
...
function renderInfo(){
  ...
  els.playersList.innerHTML = playerArray()
    .map(p => `<div class="player-seat"><strong>${escapeHtml(p.name)}</strong>${COLORS[p.color].label} ${p.uid === currentUser?.uid ? "— toi" : ""} ${isRanked(p.uid) ? "— terminé" : ""}</div>`)
    .join("") || "—";
  ...
  els.rankingList.innerHTML = gameState.ranking?.length
    ? gameState.ranking.map(r => `<li>${r.rank || ""}. ${escapeHtml(r.playerName)} — ${COLORS[r.color].label}${r.lastRemaining ? " — dernier restant" : ""}</li>`).join("")
...
els.createRoom.addEventListener("click",createRoom);els.joinRoom.addEventListener("click",joinRoom);els.startRoom.addEventListener("click",startRoom);els.leaveRoom.addEventListener("click",()=>leaveRoom(true));els.copyCode.addEventListener("click",copyCode);els.resetSelection.addEventListener("click",()=>{selectedPieceId=null;legalDestinations=[];render();});els.soundToggleOnline.addEventListener("click",toggleSound);
